...
class RAnchorGenerator(AnchorGenerator):
    ...
    def gen_single_level_auto_anchors(self,
        ...
        angles = self.angles.repeat(len(base_size))

        base_anchors = torch.stack(
            [x_center, y_center, w, h, angles], dim=-1
        )
...
class PseudoAnchorGenerator(AnchorGenerator):
    ...
    def __init__(self,
        ...
        super(PseudoAnchorGenerator, self).__init__()
...
@ANCHOR_GENERATORS.register_module()
class RotatedAnchorGenerator(DefaultAnchorGenerator):
    def __init__(self):
