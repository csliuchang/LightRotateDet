...
class DeltaXYWHBBoxCoder(BaseBBoxCoder):
    ...
    def encode(self, bboxes, gt_bboxes):
        ...
        gt_bboxes : torch.Tensor
            Target of the transformation, e.g.,
            ground-truth boxes.
    ...
    def decode(self,
        ...
        pred_bboxes : torch.Tensor
            Encoded boxes with shape
        max_shape : tuple[int], optional
            Maximum shape of boxes. Defaults to None.
...
def delta2bbox(rois,
    ...
    ----------
    rois : Tensor
        Boxes to be transformed. Has shape (N, 4)
    deltas : Tensor
        Encoded offsets with respect to each roi.
        Has shape (N, 4 * num_classes). Note N = num_anchors * W * H when
        rois is a grid of anchors.
        Offset encoding follows https//arxiv.org/abs/1311.2524.
    means : Sequence[float]
        Denormalizing means for delta coordinates
    stds : Sequence[float]
        Denormalizing standard deviation for delta coordinates
    max_shape : tuple[int, int]
        Maximum bounds for boxes. specifies (H, W)
    wh_ratio_clip : float
    ...
    -------
    Tensor
        Boxes with shape (N, 4), where columns represent
        tl_x, tl_y, br_x, br_y.
