...
__all__ = ['RUniformAssigner']


@BBOX_ASSIGNERS.register_module()
class RUniformAssigner(BaseAssigner):
    ...
    def assign(self, bboxed_pred, anchors, gt_bboxes, gt_bboxes_ignore=None, gt_labels=None):
        ...
        cost_bbox_anchors = torch.cdist(
            anchors[:, :4], gt_bboxes[:, :4],
            p=2
        )

        # TODO:  mmdetection think CPU is diff from GPU, for compute speed, we use GPU
        ...
        assign_result.set_extra_property('indexes', indexes)
        assign_result.set_extra_property('pos_gt_index', pos_gt_index)
