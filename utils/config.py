...
class Config:
    ...
    def __init__(self, cfg_dict=None, cfg_text=None, filename=None):
        ...
        for key in cfg_dict:
            if key in RESERVED_KEYS:
                raise KeyError(f'{key} is reserved for configs file')
    ...
    def _validate_py_syntax(filename):
        ...
        try:
            ...
            raise SyntaxError('There are syntax errors in configs '
                              f'file {filename}: {e}')
    ...
    def fromjson(filename):
        cfg_dict, cfg_text = Config._json2dict(filename)
        keys = cfg_dict.keys()
        for key in keys:
            if key.startswith('_') and key.endswith('_'):
                update_value_of_dict(cfg_dict, key, cfg_dict[key])
